...
const CategoryPage = ({ isAdmin, showToast, theme }) => {
  ...
  // These two variables pull the correct text and image for the top section
  const description = DESCRIPTIONS[filter !== "All" ? `${id}-${filter}` : id] || "Curated selection for the ultimate dining experience.";
  const bgImage = getBackground(id, filter !== "All" ? filter : "");

  // Logic to fetch items from Firebase
  ...
  // Filter items based on the sub-category buttons
  ...
  return (
    <div className={`min-h-screen ${theme.bgApp} ${theme.textMain} transition-colors duration-500`}>
      ...
      {/* --- RESTORED HERO SECTION (With Background) --- */}
      ...
      {/* CONTENT SECTION: Overlapping the hero image slightly */}
      <div className="max-w-7xl mx-auto px-6 -mt-12 relative z-20 pb-24">
          
          {/* SUB-CATEGORY BUTTONS (Horizontal Scroll Fixed) */}
          <div className="flex flex-col md:flex-row justify-center md:justify-between items-center mb-10 gap-6">
               <div className="flex flex-nowrap gap-2 md:gap-3 overflow-x-auto hide-scrollbar p-2 -mx-6 px-6">
               ...
               {/* Admin Add Button: Tighter for mobile */}
          ...
          {/* THE GRID: Optimized spacing and text sizes */}
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6 md:gap-10">
                  ...
                  <div key={item.id} className="group relative">
                      <Link to={`/item/${item.id}`} className="block">
                          <div className={`relative h-72 md:h-80 ${theme.cardBg} rounded-2xl overflow-hidden mb-4 border shadow-lg group-hover:shadow-2xl transition-all duration-500 group-hover:-translate-y-2 ${theme.name==='dark'?'border-white/5':'border-gray-100'}`}>
                              ...
                              {/* PRICE & NAME OVERLAY: Fixed Price Wrapping */}
                              <div className="absolute bottom-5 left-5 right-5 flex justify-between items-end">
                                  <div className="flex-1 pr-3">
                                      <h3 className="text-xl md:text-2xl font-serif text-white leading-tight drop-shadow-md">{item.name}</h3>
                                      <span className="text-[9px] uppercase font-bold text-white/50 tracking-widest">{item.type || item.subCategory}</span>
                                  </div>
                                  <span className="text-sm md:text-base font-light text-white bg-white/10 backdrop-blur-md px-3 py-1 rounded-lg border border-white/20 whitespace-nowrap">
                                      {item.price} <span className="text-[10px] font-bold">AED</span>
                                  </span>
                              </div>
                          </div>
                      </Link>

                      {/* ADMIN ACTION BUTTONS: Smaller for better mobile touch */}
                      {isAdmin && (
                        <div className="absolute top-4 right-4 flex gap-2 z-30 opacity-0 group-hover:opacity-100 transition-opacity">
                          <button onClick={(e) => { e.preventDefault(); setEditingItem(item); }} className="bg-blue-600/80 text-white p-2 rounded-full shadow-lg hover:scale-110 transition"><Edit2 size={14}/></button>
                          <button onClick={(e) => { e.preventDefault(); setDeleteId(item.id); }} className="bg-red-600/80 text-white p-2 rounded-full shadow-lg hover:scale-110 transition"><Trash2 size={14}/></button>
                        </div>
                      )}
                  </div>
              ))}
          ...
          {displayedItems.length === 0 && (
            <div className="text-center py-20 opacity-30 italic">No items found in this category.</div>
          )}
      </div>
      
      <ConfirmModal isOpen={!!deleteId} onClose={() => setDeleteId(null)} onConfirm={handleDelete} title="Delete Item" message="Are you sure you want to remove this item?" theme={theme} />
      <Footer theme={theme} />
    </div>
  );
};
